...
public class SysOrgServiceImpl extends ServiceImpl<SysOrgMapper, SysOrg> implements SysOrgService {
    ...
    @Override
    public PageInfo<SysUser> getOrgCollectors(PagingRequest pagingRequest, SysUser user, Integer orgId, String trueName) {
        PageHelper.startPage( pagingRequest.getOffset(), pagingRequest.getLimit() );
        String role = user.getRole();
        List<SysUser> collectors;
        SysUser sysUser = new SysUser();
        if (role.equals("管理员")){
            ...
            sysUser.setTrueName(trueName);
            collectors = sysOrgMapper.getOrgCollectors(sysUser);
        }else {
            sysUser.setOrgId(user.getOrgId());
            collectors = sysOrgMapper.getOrgCollectors(sysUser);
        }
