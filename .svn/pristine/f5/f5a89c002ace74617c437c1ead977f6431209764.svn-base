...
public class MszRoomServiceImpl extends ServiceImpl<MszRoomMapper, MszRoom> implements MszRoomService {
    ...
    public PageInfo<MszRoom> weProgramList(PagingRequest pagingRequest, RoomReceive roomReceive) {
        ...
        if (roomReceive.getTelId() != null && !roomReceive.getTelId().equals("")) {
            where.eq("telId", roomReceive.getTelId());
        }
        where.orderBy("status", true);
        List<MszRoom> list = super.selectList(where);
        List<MszRoom> vo = new ArrayList<>();
        for (MszRoom item : list) {
            item.setTelName(accountMapper.selectById(item.getTelId()).getName());
            if (item.getOrgId() != null) {
                item.setOrgCode(orgMapper.selectById(item.getOrgId()).getCode());
            }
            if (item.getTelId() != null) {
                item.setTelName(accountMapper.selectById(item.getTelId()).getName());
            }
            vo.add(item);
        }
...
//        }
    ...
    @Override
    public PageInfo<WeRoomsVO> getRooms(PagingRequest pagingRequest, Integer telId, String status) {
        PageHelper.startPage(pagingRequest.getOffset(), pagingRequest.getLimit());
        return new PageInfo<>(roomMapper.getRooms(status, telId));
    }

    @Override
    public List<WeRoomsVO> getSpotRooms(Integer telId) {
        return roomMapper.getSpotRooms(telId);
    }

    @Override
    public List<MszRoom> getRoomListByOrg(PagingRequest pagingRequest, Integer orgId,
                                          String telName, String houseName, String houseNumber, String userName) {
        int start = (pagingRequest.getOffset() - 1) * pagingRequest.getLimit();
        return roomMapper.getRoomListByOrg(start, pagingRequest.getLimit(), orgId, telName, houseName, houseNumber, userName);
    }


    @Override
    public int getRoomListByOrgSums(Integer orgId, String telName,
                                    String houseName, String houseNumber, String userName) {
        return roomMapper.getRoomListByOrgSums(orgId, telName, houseName, houseNumber, userName);
    }
}
