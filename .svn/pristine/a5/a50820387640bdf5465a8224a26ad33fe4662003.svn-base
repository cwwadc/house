...
public class MszWithdrawController {
    ...
    @LoginRequired
    public RespEntity updateStatus(HttpServletRequest request, @RequestParam Integer id, @RequestParam Integer operator, @RequestParam String status) {
        ...
        if (!mszWithdrawService.updateById(withdraw)) {
            sysLogUtil.insertSysLog(request, user.getId(), user.getUsername(),"提现记录 ->同意/拒绝->失败");
        ...
        if (status.equals("2")) {//提现拒绝
            ...
            message.setContentText(messageTeamplate.getContentText());
            messageService.insert(message);
            sysLogUtil.insertSysLog(request, user.getId(), user.getUsername(),"提现记录 ->同意/拒绝->提现拒绝成功");
            return RespEntity.ok("提现拒绝成功");
        }
        sysLogUtil.insertSysLog(request, user.getId(), user.getUsername(),"提现记录 ->同意/拒绝->成功");
    ...
    public RespEntity insert(@RequestBody MszWithdraw mszWithdraw) {
        ...
        if (orgId != null) {
            ...
            message.setReceiverId(3);//后台管理系统
        }
        message.setIsUser("1");
    ...
    @LoginRequired
    public RespEntity update(HttpServletRequest request, @RequestBody MszWithdraw mszWithdraw) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszWithdrawService.updateById(mszWithdraw)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"提现记录 ->修改->失败");
            return RespEntity.badRequest("更新失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"提现记录 ->修改->成功");
        return RespEntity.ok("更新成功");
    }
    ...
    public RespEntity delete(HttpServletRequest request, @PathVariable Integer id) {
        ...
        if (!mszWithdrawService.deleteById(id)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"提现记录 ->删除->失败");
            return RespEntity.badRequest("删除失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"提现记录 ->删除->成功");
