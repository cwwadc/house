...
public class MszLeaseServiceImpl extends ServiceImpl<MszLeaseMapper, MszLease> implements MszLeaseService {
    ...
    public MszLeaseServiceImpl(MszLeaseMapper mszLeaseMapper, SysCityMapper sysCityMapper, MszRoomMapper mszRoomMapper,
                               MszOrderInfoMapper mszOrderInfoMapper, MszMessageMapper messageMapper, MszRefundInfoMapper refundInfoMapper,
                               MszAccountMapper accountMapper, SysOrgMapper orgMapper, MszWithdrawMapper withdrawMapper) {
...
//            billMapper.insert(bill);
            ...
            MszAccount mszAccount = accountMapper.selectById(lease.getLandlordId());
            withdraw.setAccountId(lease.getLandlordId());
            withdraw.setCardNo(mszAccount.getOpenId());
            withdraw.setAccountName(mszAccount.getTel());
            withdraw.setAmt(paidAmount);
