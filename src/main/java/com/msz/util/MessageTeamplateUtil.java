...
public class MessageTeamplateUtil {
    ...
    public static MessageTeamplate insertLeaseByLandlord(String landlordName, String address, String tenantName, String orgName){
        teamplate.setTitle(landlordName+"，您有房源已出租");
        teamplate.setContentText("您在地址是"+address+"的房源，被"+tenantName+"的房客签订租约了，请留意交费情况！（网点名称:"+orgName+"）");
        return teamplate;
    }
    ...
    /**
     * 租金到期、交费提醒 (只发送给房客)
     * @param mszLeaseChargeList 固定收费项
     * @param rentPrice 租金
     * @param monthEndTime 本月交租时间
     * @param collectMoneyDay 交费日
     * @return
     */
    public static MessageTeamplate collectMoneyDayTenant(List<MszLeaseCharge> mszLeaseChargeList,BigDecimal rentPrice, Date monthEndTime, int collectMoneyDay){
        String str = "";
        if(mszLeaseChargeList != null){
            for (MszLeaseCharge item : mszLeaseChargeList) {
        ...
        teamplate.setContentText("您"+year+"年"+month+"月份的租约账单金额为：租金"+rentPrice+"元，"+str+"请在"+month+"月"+collectMoneyDay+"日前完成支付，逾期将影响租约的执行！");
       /*
       StringBuffer sbbuffer = null;
       if (mszLeaseChargeList == null && mszLeaseChargeList.size() ==0){
            teamplate.setContentText("您"+year+"年"+month+"月份的租约账单金额为：租金"+rentPrice+"元，请在"+month+"月"+collectMoneyDay+"日前完成支付，逾期将影响租约的执行！");
        }else {
            for (MszLeaseCharge mszLeaseCharge : mszLeaseChargeList) {
                sbbuffer.append(mszLeaseCharge.getChargeName());
                sbbuffer.append(mszLeaseCharge.getChargePrice());
            }
            teamplate.setContentText("您"+year+"年"+month+"月份的租约账单金额为：租金"+rentPrice+"元，"+sbbuffer+",请在"+month+"月"+collectMoneyDay+"日前完成支付，逾期将影响租约的执行！");
        }*/
        return teamplate;
    }
   ...
   //--------------------------------------------------欠租-----------------------------------------------
     ...
     * @return
     */
    public static MessageTeamplate arrearsOrderTenant(String orderNo,Date endDate, BigDecimal total){
        teamplate.setTitle("欠租通知");
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(endDate);
        int year = calendar.get(Calendar.YEAR);//年
        int month = calendar.get(Calendar.MONTH)+1;//月
        int day = calendar.get(Calendar.DAY_OF_MONTH);//日
        teamplate.setContentText("您的订单编号为"+orderNo+"的订单已经欠租，欠收金额为"+total+"元,请在"+year+"年"+month+"月"+day+"日之前交清所欠房租！");
        return teamplate;
    }
     ...
     * @return
     */
    public static MessageTeamplate collectMoneyDayMessage(String orderNo,Integer month, BigDecimal total){
        teamplate.setTitle("逾期交费通知");
        teamplate.setContentText("您"+month+"月份订单编号为"+orderNo+"租约的已经逾期，逾期金额为"+total+"元，请您及时还款，避免影响您的个人信用！");
        return teamplate;
    }
     ...
     * @return
     */
    public static MessageTeamplate collectMoneyDayMessage(String receiveName,Integer month, String leaseNo, BigDecimal total){
        teamplate.setTitle("逾期交费通知");
        teamplate.setContentText("您的房客"+receiveName+month+"月份订单编号为"+leaseNo+"的订单已经逾期，逾期金额为"+total+"元，请通知他及时还款！");
        return teamplate;
    }
