...
public class MszWalletController {
    ...
    @LoginRequired
    public RespEntity insert(HttpServletRequest request,  @RequestBody MszWallet mszWallet) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszWalletService.insert(mszWallet)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->新增->失败");
            return RespEntity.badRequest("保存失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->新增->成功");
    ...
    @ApiOperation(value = "根据ID修改MszWallet", notes = "根据ID修改MszWallet")
    @LoginRequired
    public RespEntity update(HttpServletRequest request, @RequestBody MszWallet mszWallet) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszWalletService.updateById(mszWallet)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->更新->失败");
            return RespEntity.badRequest("更新失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->更新->成功");
        return RespEntity.ok("更新成功");
    }
    ...
    public RespEntity delete(HttpServletRequest request, @PathVariable Integer id) {
        ...
        if (!mszWalletService.deleteById(id)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->删除->失败");
            return RespEntity.badRequest("删除失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"用户钱包 ->删除->成功");
