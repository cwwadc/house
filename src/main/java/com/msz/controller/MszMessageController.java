...
public class MszMessageController {
    ...
    @LoginRequired
    public RespEntity insert(HttpServletRequest request,  @RequestBody MszMessage mszMessage) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszMessageService.insert(mszMessage)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"消息->新增->失败");
            return RespEntity.badRequest("新增失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"消息->新增->成功");
    ...
    public RespEntity update(HttpServletRequest request, @RequestBody MszMessage mszMessage) {
    ...
    public RespEntity updateIsDel(HttpServletRequest request, @PathVariable Integer id) {
        ...
        if (!mszMessageService.updateById(mszMessage)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"消息->删除->失败");
            return RespEntity.badRequest("删除失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"消息->删除->成功");
