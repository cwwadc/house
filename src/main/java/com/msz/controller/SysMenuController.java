...
public class SysMenuController {
    ...
    @LoginRequired
    public RespEntity insert(HttpServletRequest request, @RequestBody SysMenu sysMenu ){
        SysUser sysUser = UserCommon.getCurrentUser();
        if ( ! sysMenuService.insert( sysMenu) ){
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"菜单 ->新增->失败");
            return RespEntity.badRequest("保存失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"菜单 ->新增->成功");
    ...
    @LoginRequired
    public RespEntity update(HttpServletRequest request, @RequestBody SysMenu sysMenu ){
        SysUser sysUser = UserCommon.getCurrentUser();
        if ( ! sysMenuService.updateById( sysMenu ) ){
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"菜单 ->更新->失败");
            return RespEntity.badRequest("更新失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"菜单 ->更新->成功");
