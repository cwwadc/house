...
//    }
    ...
    @LoginRequired
    public RespEntity insert(HttpServletRequest request, @RequestBody MszPayInfo mszPayInfo) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszPayInfoService.insert(mszPayInfo)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->新增->失败");
            return RespEntity.badRequest("保存失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->新增->成功");
    ...
    @LoginRequired
    public RespEntity update(HttpServletRequest request, @RequestBody MszPayInfo mszPayInfo) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszPayInfoService.updateById(mszPayInfo)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->更新->失败");
            return RespEntity.badRequest("更新失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->更新->成功");
    ...
    @LoginRequired
    public RespEntity delete(HttpServletRequest request, @PathVariable String id) {
        SysUser sysUser = UserCommon.getCurrentUser();
        if (!mszPayInfoService.deleteById(id)) {
            sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->删除->失败");
            return RespEntity.badRequest("删除失败");
        }
        sysLogUtil.insertSysLog(request, sysUser.getId(), sysUser.getUsername(),"支付信息->删除->成功");
