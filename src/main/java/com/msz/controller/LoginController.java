...
public class LoginController {
    ...
    public RespEntity login(HttpServletRequest request,String username, String password) throws Exception {
        ...
        if (sysUser != null) {
            String md5Password = MD5Utils.MD5Encode(username, password);
            if (MD5Utils.verify(sysUser.getPassword(), md5Password) == true) {
...
//                    redisUtil.set("user", user);
    ...
    @GetMapping("/test")
    @LoginRequired
    public RespEntity testLogin(@CurrentUser SysUser user){
    ...
    private String creatToken(SysUser user, Date date) {
        ...
        JwtBuilder builder = Jwts.builder().setHeaderParam("typ", "JWT") // 设置header
                ...
                .setExpiration(new Date(date.getTime() + 1000 * 60 * 120))
